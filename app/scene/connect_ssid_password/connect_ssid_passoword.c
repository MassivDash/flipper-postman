...
void uart_terminal_scene_text_input_callback(void* context) {
    ...
    view_dispatcher_send_custom_event(app->view_dispatcher, AppEvent_Connect_Ssid_Password);
}
...
bool scene_on_event_connect_ssid_password(void* context, SceneManagerEvent event) {
    App* app = context;
    bool consumed = false;
