...
void submenu_callback_set_password(void *context, uint32_t index) {
  ...
  FURI_LOG_I(TAG, "Setting password for WiFi");
}
...
void scene_on_enter_connect_details(void *context) {
  ...
  submenu_add_item(app->submenu_wifi,
                   app->status == BOARD_CONNECTED_WIFI_ON ? "Disconnect"
                                                          : "Connect",
                   0, submenu_callback_disconnect_connect, app);
  submenu_add_item(app->submenu_wifi, "Set Password", 1,
                   submenu_callback_set_password, app);
  submenu_add_item(app->submenu_wifi, "Save to flipper", 2,
                   submenu_callback_save_to_csv, app);
  submenu_add_item(app->submenu_wifi, "Mark as default", 3,
                   submenu_callback_save_to_csv, app);
  submenu_add_item(app->submenu_wifi, "Forget network", 4,
                   submenu_callback_save_to_csv, app);
