...
void scene_on_enter_post_url_list(void* context) {
    ...
    if(app->url_list[0].url[0] != '\0') {
        submenu_reset(app->submenu);
        submenu_set_header(app->submenu, "Available URLs");

        // Add URLs to submenu
        for(size_t i = 0; i < MAX_URLS && app->post_url_list[i].url[0] != '\0'; i++) {
            ...
            submenu_add_item(app->submenu, display_name, i, submenu_callback_select_post_url, app);
        }
    } else {
        submenu_reset(app->submenu);
