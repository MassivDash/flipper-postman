...
bool init_csv(File* file, WifiCredential* csv_networks) {
    ...
    if(storage_file_exists(storage, APP_DATA_PATH("wifi.csv"))) {
        ...
        if(!sync_csv_to_mem(file, csv_networks)) {
...
bool sync_csv_to_mem(File* file, WifiCredential* csv_networks) {
    ...
    while(read_line_from_file(file, buffer) && credential_index < MAX_WIFI_CREDENTIALS) {
...
bool find_and_replace_wifi_in_csv(File* file, const WifiCredential* current_wifi) {
    ...
    while(read_line_from_file(file, buffer_wifi)) {
        WifiCredential wifi = {0};
