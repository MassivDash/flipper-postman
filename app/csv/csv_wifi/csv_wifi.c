...
bool init_csv_wifi(App* app) {
    ...
    // Allocate file
    app->file_wifi = storage_file_alloc(app->storage);
    ...
    if(storage_file_exists(app->storage, APP_DATA_PATH("wifi.csv"))) {
        ...
        if(!sync_csv_to_mem(app, app->csv_networks)) {
            ...
            storage_file_close(app->file_wifi);
            return false;
        }
    } else {
        FURI_LOG_I(TAG, "CSV file does not exist, creating a new one.");
        if(!storage_file_open(
               app->file_wifi, APP_DATA_PATH("wifi.csv"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
...
bool sync_csv_to_mem(App* app, WifiCredential* csv_networks) {
    ...
    if(!storage_file_open(
           app->file_wifi, APP_DATA_PATH("wifi.csv"), FSAM_READ, FSOM_OPEN_EXISTING)) {
    ...
    while(read_line_from_file(app->file_wifi, buffer) && credential_index < MAX_WIFI_CREDENTIALS) {
...
bool write_wifi_to_csv(App* app, const WifiCredential* wifi) {
    ...
    if(!storage_file_open(
           app->file_wifi, APP_DATA_PATH("wifi.csv"), FSAM_WRITE, FSOM_OPEN_APPEND)) {
    ...
    if(!storage_file_write(
           app->file_wifi, furi_string_get_cstr(buffer), furi_string_size(buffer))) {
...
bool delete_wifi_from_csv(App* app, const char* ssid) {
    ...
    while(read_line_from_file(app->file_wifi, buffer_wifi)) {
        WifiCredential wifi = {0};
